...
- On finish, `WorkoutScreen.handleSubmit`:
  ...
  2. **Updates `users.xp` / `users.level` / `users.streak`** in one combined write, folding in the workout XP, the daily-streak bonus, and any newly-earned medal XP — a single source of truth for the profile update rather than multiple writes racing each other.
     - Streak (`users.streak`, daily): last-session-date logic — consecutive day → `+1`, same day → unchanged, gap → reset to `1`.
...
- **Daily-streak bonus** — `+2 XP` (`DAILY_STREAK_XP`), but only the *first* completion of a given calendar day (checked against `priorHistory` — a second workout the same day doesn't re-trigger it, matching how the streak counter itself doesn't move on a same-day repeat either).
...
- **Catalog rebuilt from the "Achievements & XP Values" spec — a curated ~20-item subset of its full ~90-item list**, not all of it (see Deferred below). XP tiers per the spec: 10 (quick wins/onboarding), 15 (building momentum), 20 (streaks & consistency), 30 (advanced effort), 50 (major milestones). Current catalog (`mockMedals`, `mockData.ts`), all checked in `evaluateAndAwardMedals` (db.ts) unless noted:
  - **10 XP:** `1` First Step (first workout), `6` Early Bird (first workout completed before noon local time), `8` Night Owl (first workout completed 6pm+ local time), `9` Profile Complete (birth_year/sex/height_cm/activity_level all set), `11` Progress Logged (first weight log entry), `10` Coach Connected (awarded from `acceptCoachRequest`, not here — see below).
  - **15 XP:** `12` 3-Day Streak, `13` Plan Follower (5 workouts), `14` 10K Steps (single-day max), `15` Daily Doer (10 distinct active days).
  - **20 XP:** `2` 7-Day Streak, `16` 10 Workouts Strong, `17` Next Level (2+ workouts ever assigned, i.e. `getWorkoutsForTrainee(...).length >= 2`).
  - **30 XP:** `18` 14-Day Streak, `19` 21-Day Streak, `20` 25 Workouts Strong.
  - **50 XP:** `3` 30-Day Streak, `4` 100 Workouts, `21` 50 Workouts Strong, `22` Always Moving (100 distinct active days).
  - **Not automated:** `5` Top Ranker (needs a leaderboard-rank query), `7` New Adventure (kept only so a trainee who already earned it under the old catalog keeps seeing it — shares `1` First Step's exact `sessionsCount >= 1` trigger, so re-enabling it would double-fire on the same event).
- **`evaluateAndAwardMedals(userId, streak)`** — signature changed (dropped the `sessionsCount` param). It now recomputes an accurate lifetime session count itself via `getWorkoutSessionCount(traineeId)` (a `count`-only Supabase query), fixing a real pre-existing bug: the old approach used `priorHistory.length + 1` where `priorHistory` came from `getTraineeHistory`'s **default limit of 20** — meaning the 25/50/100-workout milestones could *never* have fired correctly for any trainee with more than ~20 sessions. New helper queries in `db.ts`, all doing their own full-history scan (not capped like `getTraineeHistory`): `getWorkoutSessionCount`, `getDistinctActiveDayCount` (distinct calendar days with a session — two workouts the same day count once), `hasCompletedInHourRange(traineeId, minHour, maxHour)` (morning/evening detection), `getMaxDailySteps` (highest single-day `vitals` steps entry ever).
...
- `GamificationScreen.tsx` merges the static `mockMedals` definitions (name/icon/rarity/xpReward) with real earned state from `getUserMedals(userId)` — unchanged mechanism, just a bigger catalog now; its medals grid is a plain `flexWrap` layout, so it scaled from 7 to ~22 entries with no layout changes needed.

### Deferred achievements (from the ~90-item spec — not silently dropped, just not buildable yet)

Roughly half the full spec was left out of the catalog above. Two categories:

1. **Near-duplicates under this app's current model** — the spec lists several achievements that collapse onto the exact same trigger condition here (no self-directed vs. coached workout distinction, no workout-type categorization), so only one representative was implemented per group rather than multiple medals co-firing on one event: *Welcome Aboard / Plan Activated / Logged & Done / Plan Starter / First Rep / Coach Approved* (all = first workout, same as First Step); *Consistency King* (= 10 Workouts Strong); *Consistency Pro / Data Devotee* (= 25/50 Workouts Strong, picking the tier the majority of the spec's own duplicate entries agreed on); *Tracking Pro* (= 25 Workouts Strong); *Stronger Than Before / Progressive* (= would-be "Personal Best," see below).
2. **Needs a feature that doesn't exist yet** — flagged here as backlog, not attempted:
   - **Weight-based PRs** (`Progressive`, `Personal Best`, `PR Hunter`, `PR Collector`): `exercise_weight_logs` entries are written from the **coach-assigned weight** (`ex.coachWeight`), not anything the trainee enters — see "Workout Screen (Trainee)" above, weight is display-only for the trainee. Building this would reward a *coach's* decision to raise prescribed weight, not the trainee's own performance, so it was skipped rather than implemented with misleading semantics. Needs either trainee-editable performance logging or a dedicated coach-initiated weight-progression feature first.
   - **Coach check-ins / feedback / challenges as trackable events** (`First Check-In`, `Feedback Friend`, `Question Asked`, `Coach's Challenge`, `Team Player`, `Coached Consistency`, `Stronger Together`, `Go The Distance`, `Feedback Applied`, `Coach Check-In`): the `messages` table exists but isn't structured as check-ins/feedback/challenges — no distinct event types to key off of.
   ...
   - **Mobility / recovery / rest-day session types** (`Stretch Starter`, `Cool Down`, `Rest Day Respect`, `Recovery Matters`, `Recovery Pro`, `Recovery Streak`, `Balance Builder`, `Flexible Future`): workouts have no category (Strength/Cardio/Mobility/Recovery) — every workout is just "a workout."
   - **Distance/running** (`First Mile`, `Cardio Starter`, `5K Finisher`, `10K Finisher`, `Distance Builder`, `Long Haul`, `Endurance Builder`, `Cardio Regular`, `Strength Builder`, `Strength Machine`, `Iron Beginner`): no GPS/distance tracking, same deferral as the running-XP items above.
   - **Cumulative training hours** (`100 Hours`, `250 Hours`): would need per-session duration to be *persisted* (currently only computed transiently for the duration-bonus XP, then discarded) — needs a new `workout_sessions` column + migration.
   - **Weekly/monthly schedule-adherence percentages** (`On Track`, `Perfect Week`, `Week Warrior`, `Month Master`, `Perfect Month`, `Never Miss Twice`, `7-Day Mover`, `Movement Month`): this is essentially the removed `evaluateWeeklyCompletion` engine's logic — buildable (the algorithm already existed once this session before being retired), just not rebuilt in this pass given everything else already in scope.
   - **Ambiguous/undefined thresholds** (`Move More`, `Step Up`, `Active Day`, `Weekend Warrior`, `Active Hour`, `Walk It Out`, `Data Driven`, `Trend Setter`, `Proof of Progress`, `Progress Month`, `Progress Check`, `Progress Revealed`, `Coach's Milestone`): no defined baseline/threshold in the spec to compute against (e.g. "daily activity goal" isn't a value that exists anywhere).
   - **Long-horizon tenure** (`Committed`, `Transformation Journey`, `One Year Strong`, `Plan Complete`): "12 weeks of structured training" / "finish your first training plan" aren't states this app's data model can currently evaluate precisely.
...
- ~45 achievements from the "Achievements & XP Values" spec are deliberately not implemented yet (near-duplicates under this app's model, or need features that don't exist — weight-based PRs, coach check-ins/feedback/challenges, goals, sleep, progress photos, mobility/recovery/rest-day categorization, distance/running, cumulative training hours, weekly/monthly schedule-adherence %) — full breakdown in the Medals section's "Deferred achievements" list.
