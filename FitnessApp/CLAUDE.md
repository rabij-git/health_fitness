...
- **`TrainerDashboard.tsx` (Home):** the workout preview card fetches `getWorkoutsForTrainee` and previews the most recent *active* one (`primaryWorkout`) — top 3 exercises, no overflow count. **Deliberately non-interactive** — an earlier version of this card made the whole thing tappable to jump to the Workout tab, but that read as misleading since tapping it silently launched the workout-picker flow. Reverted to a plain preview; picking/starting a workout only happens via the bottom Workout tab button.
...
- **Schema:** `nutrition_plans` (per-trainee instances) carries both the original PDF fields (`file_name`/`file_url`/`storage_path`, all nullable — not every plan has a document) and structured fields (`title` not null, `notes`, optional `target_calories`/`target_protein`/`target_carbs`/`target_fat`, `active` boolean default true, `template_id` nullable FK). **A trainee can have several nutrition plans** — some active, some inactive — mirroring `workouts.active` exactly. Migrations: `/private/tmp/scratch/merge_nutrition_diet_plans.sql`, `/private/tmp/scratch/nutrition_templates_and_workout_enddate.sql` (both already run).
...
- **Per-trainee assignment (`CoachTrainees.tsx`):** the trainee-detail modal's "Nutrition" tab is an expandable list (same pattern as the Program tab's workout list) with Edit/Active-toggle/Delete per plan. Two actions up top: "Upload PDF" (quick, unchanged) and "Assign Plan" (opens a picker of the coach's templates — tap one to assign). The old ad-hoc "New Plan" (build from scratch, no template) button is gone.
