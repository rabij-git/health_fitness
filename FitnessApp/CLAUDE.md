...
- **Trainee tabs** (`TrainerTabs.tsx`, note: "Trainer" here is the trainee-facing role name, confusingly): Home, Workout, Log, Medals, Social, Profile.
  - `Log` is a segmented control internally (Exercises / Nutrition), not two separate tabs — see Nutrition Plans & Food Log below.
...
- **`TrainerDashboard.tsx`'s "Calories Today" card was previously 100% hardcoded** — `—`/"not synced"/"Synced by your coach"/a permanent 0%-width bar, regardless of any real plan or logging — a leftover from before nutrition tracking existed for trainees. Now wired to the same source as `FoodLogScreen.tsx`'s Nutrition tab (see below): today's calories (manual `food_log_entries` + `sumTodayAsPlannedCalories`) against the active plan's `target_calories`, a live-filled progress bar, and the plan's target macros (labeled "DAILY TARGETS", not framed as consumed — see Nutrition Plans & Food Log). Honest empty state — "No active nutrition plan" — when there isn't one, instead of the old fake "not synced". **Over-budget state:** when today's calories exceed the target, the "kcal left" figure switches to "kcal over" (showing the absolute overage) and both the number and the progress bar render in `colors.primary` (red) instead of the normal color — mirrored identically in `FoodLogScreen.tsx`'s own Calories Today card. Both cards also show a small colored macro-target row (protein/carbs/fat, each with its own dot color) beneath the calorie bar when the plan has any target macro set.
...
- **Trainee side:** no new bottom tab was added (6 tabs was already the ceiling) — the existing **Log tab is a segmented control** (`LogScreen.tsx`): "Exercises" (`ExerciseLogScreen.tsx`, read-only — see Workout Completion Flow) and "Nutrition" (`FoodLogScreen.tsx`) — shows Active/Past nutrition plan cards (target macro chips including water, notes, tappable PDF link if attached) plus the day-by-day food log below (grouped "Today"/"Yesterday"/weekday, view/delete only — the manual "Add Food" entry point was removed per feedback; any calories a trainee logs now come exclusively from marking generated meals "As Planned", see `sumTodayAsPlannedCalories` below). No "No food logged yet" placeholder text when the log is empty — the section is simply blank, since the day-grouped list itself already communicates emptiness without needing a caption. If any active plan has `target_calories` set, shows a "today's calories / target" progress bar; if any active plan has `target_water_ml` set, a second progress bar shows today's actual water intake (`getTodayMetrics(userId).water_ml`, the same phone-independent manual/synced total used on the trainee's Home screen) against that target. `LogScreen.tsx` owns the single `SafeAreaView` for the tab; `ExerciseLogScreen.tsx`/`FoodLogScreen.tsx` use plain `View` so insets aren't applied twice (neither is ever rendered standalone).
