...
- **No DB credentials are available in the Claude Code environment itself** — schema changes (new tables/columns) require handing the user a `.sql` file (put it in `scripts/`, e.g. `scripts/reset_trainee_test_data.sql`) to run in the Supabase SQL Editor. **Real per-table RLS is now enforced** (see "Database Tables" below) — the anon key alone, without a genuine logged-in user's JWT, now gets zero access to anything. There is no more casual "just read/write via the anon key" path; any data read/write from this environment has to go through the same hand-a-`.sql`-file-to-the-user workflow as a schema change.
...
**All tables have real, per-table RLS** — scoped to `auth.uid()`/`coach_id`/`trainee_id`/`role`, not allow-all (fixed this session; migration script has since been deleted locally, already applied). Closes: any authenticated anon-key user self-promoting to `role: 'admin'` via a raw `users` PATCH; open coach signup / forged coach-invite redemption; cross-trainee reads/writes on vitals/workouts/nutrition/messages/etc. Policies are scoped to `auth.uid()`/`coach_id`/`trainee_id`, with an admin-bypass baked into every coach-ownership check so "Login as &lt;coach&gt;" impersonation keeps working, plus a trigger blocking any non-admin `users.role` change. Two flows needed server-side atomicity instead of a plain client read/write, now SECURITY DEFINER RPCs: `redeem_coach_invite` (coach signup — `signUpCoach()` calls it instead of reading/updating `coach_invites` directly) and `accept_coach_request` (`acceptCoachRequest()` calls it instead of a raw `users` update; the old `assignTraineeToCoach()` helper was removed, superseded by this). `signUp()` no longer takes a `role` param — always inserts as `trainee`; only `redeem_coach_invite` can produce a `coach` row.
...
**Resetting a test trainee's data:** `scripts/reset_trainee_test_data.sql` — clears one trainee's workouts/nutrition/vitals/messages/medals and resets xp/level/streak to 0, without touching their account, login, or coach connection. Edit the email at the top before running it in the Supabase SQL Editor.
...
## Pending / Future Work
- **Security, not yet addressed (found during the RLS review — see "Database Tables" above for what *was* fixed):** `uploadFileToStorage` (`db.ts`) authenticates storage uploads with the hardcoded anon key as the bearer token instead of the caller's real session `access_token` — harmless while storage bucket policies are themselves unexamined/possibly-permissive, but should be fixed together with an actual look at the `nutrition-plans` bucket's storage.objects policies (deliberately scoped out of the RLS lockdown to avoid touching bucket policies blind).
