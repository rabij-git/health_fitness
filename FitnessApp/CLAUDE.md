...
- On finish, `WorkoutScreen.handleSubmit`:
  ...
  5. Auto-sends a message to the trainee's coach (see Coach Notifications below) — the "+N XP" clause is now omitted entirely when `totalXp` is `0` (a very ordinary outcome now, not an edge case), rather than showing "+0 XP".
...
- Trainee finishing a workout auto-sends a message to their coach (`🏋️ {name} completed "{workout}" — {pct}% done, +{xp} XP` — the `, +{xp} XP` clause is omitted entirely when `xp` is `0`, which is now the common case since a single workout rarely earns any on its own; see the XP rework in "Workout Completion Flow" above).
