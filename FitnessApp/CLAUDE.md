...
- On finish, `WorkoutScreen.handleSubmit`:
  1. `saveWorkoutSession(...)` — `xp_awarded` is now always `0` (see "XP rework" below — finishing a single workout no longer awards XP directly, so there's nothing session-specific to record here).
  2. **Updates `users.xp` / `users.level` / `users.streak`** in one combined write, folding in both the medal-bonus XP and the weekly-completion XP (see below) so there's a single source of truth for the profile update instead of two separate writes racing each other.
     - Streak (`users.streak`, daily): last-session-date logic — consecutive day → `+1`, same day → unchanged, gap → reset to `1`. Unaffected by the XP rework — still updates on every finish regardless of whether XP was earned.
     - Level: `computeLevelFromXp(xp) = floor(xp / 500) + 1` (`mockData.ts`).
  3. `evaluateAndAwardMedals(userId, sessionsCount, newStreak)` — see Medals below.
  4. `evaluateWeeklyCompletion(userId)` (`db.ts`) — see "XP rework" below.
  5. Auto-sends a message to the trainee's coach (see Coach Notifications below) — no XP/points mentioned at all, per feedback that coaches don't need score detail, just that the client did the work.
- Workout count on dashboards is derived live from `workout_sessions` rows — no separate counter needed.

### XP rework — weekly completion replaces per-workout XP

Per feedback, finishing a single workout **no longer awards XP at all** (previously a flat `+= Math.round(250 * progress)` on every finish). Replaced with a once-per-week bonus:

- **Schema:** `users.weekly_streak` (int, default `0`) and `users.last_completed_week_start` (date, nullable) — migration `/private/tmp/scratch/users_weekly_streak.sql` (already run). Separate from `users.streak` above, which is a *daily* (any-workout-that-day) consecutive-day count; `weekly_streak` counts consecutive Sun–Sat weeks with **every scheduled day of every active, scheduled workout** completed.
- **`evaluateWeeklyCompletion(userId)` (`db.ts`):** only considers workouts with an explicit `scheduled_days` set (an unrestricted any-day workout has no weekly cadence to check against). Computes the current week's Sunday start; bails out (no-op, not a failure) if that week was already credited (`last_completed_week_start` matches), or if any scheduled workout still has a scheduled day later this week that hasn't happened yet (`Math.max(scheduled_days) > today's weekday` — too early to judge). Once every remaining required day has occurred, checks `workout_sessions` for that week and requires **every** scheduled day of **every** qualifying workout to have a completed session on that specific date. On success: awards `XP_PER_COMPLETED_WORKOUT = 1` XP **per completed daily workout that week** (sum of `scheduled_days.length` across all qualifying workouts — a 3-workout week nets 3 XP, a 5-workout week nets 5), all released together only once the whole week is confirmed done, not incrementally per day as each one is completed — per feedback, points are earned per daily workout but only *paid out* at week's end, not a flat per-week bonus regardless of workout count (an earlier version awarded a flat +10/week; changed to scale with actual workout count instead). Also sets `weekly_streak` to `+1` if `last_completed_week_start` was exactly the prior week (else resets to `1`), and stamps `last_completed_week_start`. **Deliberately does not write `xp`/`level` itself** — only the `weekly_streak`/`last_completed_week_start` bookkeeping — so it can't race with `WorkoutScreen.handleSubmit`'s own combined xp/level write for the medal bonus; it just returns `{ awarded, weeklyStreak, xpAwarded }` for the caller to fold in.
- **Medals still fire independently** (7-Day Streak, 30-Day Streak, 100 Workouts) and their (now rescaled) XP is added on top of the weekly-completion XP in the same combined write — see Medals below.
- **`ProfileScreen.tsx`** shows the new `weekly_streak` as a "Weekly Streak" stat tile (`{n}wk`) alongside the existing "Day Streak" tile — both visible, clearly two different counters.
- **Completion modal (`WorkoutScreen.tsx`)** shows a "Full Week Completed!" callout (reusing the medal-unlocked visual treatment) whenever `evaluateWeeklyCompletion` reports `awarded: true`, alongside whatever medal(s) also fired that session.
...
- `evaluateAndAwardMedals` (db.ts) checks the objectively-computable rules and awards any newly-qualified ones after each workout finish:
  - `2` 7-Day Streak (streak ≥ 7), `3` 30-Day Streak (streak ≥ 30), `4` 100 Workouts (sessions ≥ 100).
- **`1` First Workout and `7` New Adventure are no longer auto-awarded** — both used to fire on `sessionsCount >= 1`, i.e. for finishing a single workout, which per feedback is no longer how achievements work (see the weekly-completion reward in "Workout Completion Flow" above). They're still defined in `mockMedals` (so a trainee who already earned one keeps seeing it on their profile) but nothing awards them going forward — don't reintroduce the `sessionsCount >= 1` checks.
...
- `GamificationScreen.tsx` merges the static `mockMedals` definitions (name/icon/rarity/xpReward) with real earned state from `getUserMedals(userId)` — `mockMedals` itself is unchanged in shape, only the `earned` flag is real.
- **XP rewards rescaled to 10–50** across the whole catalog (common lowest, ultra_rare highest) — previously ranged 50–3000, wildly out of proportion once the old flat 250-XP-per-workout base was removed. Current values: First Workout 10, New Adventure 10, Early Bird 15, 7-Day Streak 25, 100 Workouts 35, Top Ranker 45, 30-Day Streak 50.
